import pytest
from csp_tool.csp_analyser import (
    get_domains_per_directive,
    rollup_data_by_header,
    check_unsafe_inline_used_without_domains,
    extract_policies_without_domains,
    check_keyword_used_without_domains,
    check_keyword_used_with_domains,
    extract_policies_using_localhost,
)
...
def test_rollup_data_by_header(
    header, expected_num_rows, biggest_count_row, expected_count
):
    ...
    analysed = rollup_data_by_header(
        data, header, active_only=False, headers=data_headers
    )
...
def test_reported_ok_no_unsafe_inline():
    ...
    csp = {
        "default-src": {"keywords": ["self"], "domains": ["www.google.com"], "data": []}
    }
...
def test_reported_found_unsafe_inline_without_domains():
    ...
    csp = {"default-src": {"keywords": ["unsafe-inline"], "domains": [], "data": []}}
    unsafe_without_domains = check_unsafe_inline_used_without_domains(csp)
    assert expected_result == unsafe_without_domains
...
def test_reported_ok_no_unsafe_eval_no_domains():
    expected_result = False
    csp = {
        "default-src": {"keywords": ["self"], "domains": ["www.google.com"], "data": []}
    }
...
def test_reported_ok_unsafe_eval_with_domains():
    ...
    unsafe_with_domains = check_keyword_used_with_domains("unsafe-eval", csp)

    assert expected_result == unsafe_with_domains


def test_reported_ok_unsafe_eval_without_domains():
    expected_result = False
    csp = {
        "default-src": {
...
def test_reported_found_unsafe_eval_without_domains():
    expected_result = True
    csp = {"default-src": {"keywords": ["unsafe-eval"], "domains": [], "data": []}}
    unsafe_without_domains = check_keyword_used_without_domains("unsafe-eval", csp)
    assert expected_result == unsafe_without_domains


def test_reported_found_unsafe_eval_with_domains():
    expected_result = True
    csp = {
        "default-src": {
            "keywords": ["unsafe-eval"],
